...
const CompanyRegister = () => {
  ...
  const registerStudentFunc = (values, { resetForm }) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === "logo") {
        ...
        if (value instanceof File) {
          formData.append(key, companyLogo);
        }
      } else {
    ...
    axios
      .post(`${API_BASE_URL}/company_register/`, formData)
      .then((value) => {
        toast.success(
          "You are registerd Successfully. Verify Link Send to your email",
          {
            position: "top-center",
          }
        );
                    ...
                    if (element.type == "checkbox") {
                      return (
                        ...
                        >
                          ...
                          />
                          &nbsp;&nbsp;{" "}
                          <h4 className="text-blue-600 mb-2 inline-block mt-4">
                            {element.placeholder}{" "}
                          </h4>
                          <ErrorMessage
                            name={element.name}
